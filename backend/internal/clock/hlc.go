...
func (c *HLC) Observe(remote model.Version) (model.Version, error) {
	...
	localPhysical := last.PhysicalMillis
	...
	case physical == localPhysical:
		logical = nextLogical(&physical, last.Logical)
	...
	next := model.Version{PhysicalMillis: physical, Logical: logical, OriginNodeID: c.nodeID}
	c.mu.Lock()
	c.last = next
	c.mu.Unlock()
	return next, nil
